...
def _tessellate_to_trimesh(shape) -> trimesh.Trimesh:
    ...
    while explorer.More():
        ...
        if triangulation is not None:
            ...
            # Cut/Fuse (or, as it turns out, just one of a plain
            # BRepPrimAPI_MakeBox's own six faces — box primitives are not
            # uniformly FORWARD) keeps the underlying surface's natural
            # winding, which is backwards relative to the solid's outward
            # normal. Left uncorrected, three.js's default backface culling
            # makes that triangle invisible from outside — at some rotation
            # angles the camera ray grazes straight through it, reading as
            # "the body went see-through". Flip two indices to restore
            # outward winding whenever the face is REVERSED.
